class Bats {
    ...
    function spawnBats() {
        ...
        if (player.y <= 150) {
            obstacle = new Bats2(cWidth - 30, Math.floor(cHeight * 0.5), size, size);
        } else {
            obstacle = new Bats(cWidth + size, Math.floor(cHeight * 0.22) - size, size, size);
        }
    ...
    function spawnNextBat() {
        ...
        if (player.y <= 150) {
            obstacle = new Bats2(cWidth + 270, Math.floor(cHeight * 0.42), Math.floor(Math.random() * 31) + 60, Math.floor(Math.random() * 31) + 60);
        } else {
            obstacle = new Bats(cWidth + 270 + size, Math.floor(cHeight * 0.33) - size, Math.floor(Math.random() * 31) + 60, Math.floor(Math.random() * 31) + 60);
        }
        Math.random(obstacles.push(obstacle) + 1);
    }
